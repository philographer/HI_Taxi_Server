...
exports.checkStation = function(req, res){

    var station = req.query.station;
    var express = req.query.express;
    var holiday = req.query.holiday;

    dbService.readStation(station ,express, holiday).then(function(result){
        res.status(200).json(result);
    }).catch(function(error){
        res.status(500).json({message: error.message});
    });
