...
DbService.prototype.validimei = function(imei){
    ...
    this.model['user'].find({where: {imei: imei}}).then(function (res){
        ...
        }else if(res.body.verified == true){
...
DbService.prototype.insertData = function(){
    var that = this;
    var rFile = __dirname + '/exholiday.json';
    jsonfile.readFile(rFile, function(err, objs){
        ...
        objs['result'].forEach(function(obj, index){
            ...
            for (var i in obj){
                //i: key값 ex station, k1123123, k123123....
                //
                if(i != 'station' && obj[i]){
                    that.model['station'].create({station: this_station, name: i, time: obj[i], express: true, holiday: true});
                }
            }
        });
    });
