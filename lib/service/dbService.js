...
DbService.prototype._createVerification = function(email){
    var deferred = Q.defer();
    var input = email;
    var token = CryptoJS.AES.encrypt(input,config.SECRET);

    this.model['verification'].create({email: email, verification_code: token.toString()}).then(function(res){
...
DbService.prototype.emailVerification=function (token) {
    var deferred = Q.defer();
    console.log(token);
